...
import { useApiGameEngine } from '../hooks/useApiGameEngine';
...
const Terminal: React.FC = () => {
  ...
  const getLineClassName = (type: OutputLine['type']) => {
    switch (type) {
      case 'narrative':
        return 'text-terminal-amber'; // Server messages - yellow
      case 'error':
        return 'text-terminal-red';
      case 'system':
        return 'text-terminal-amber'; // Server messages - yellow
      case 'command':
        return 'text-terminal-dim'; // User input - white
      case 'ascii':
  ...
  return (
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="terminal-input-line">
        <span className="text-terminal-dim mr-2 shrink-0">{'>'}</span>
